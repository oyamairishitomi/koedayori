...
class PostsControllerTest < ActionDispatch::IntegrationTest
  test "正しいslugに音声を送るとPostが作成される" do
    ...
    assert_difference "Post.count", 1 do
      post "/speakers/#{speaker.slug}/posts", params: { audio: audio_file, theme_id: themes(:one).id }
    end
  ...
  test "存在しないslugに送ると404になる" do
    ...
    assert_no_difference "Post.count" do
      post "/speakers/nonexistent-slug/posts", params: { audio: audio_file, theme_id: themes(:one).id }
    end

    assert_response :not_found
  end

  test "受け取り停止中のSpeakerには投稿できない" do
    family = Family.create!(email: "test@example.com", aikotoba: "aaa", password: "password123")
  ...
  test "音声ファイルがないとPostが作成されない" do
    ...
    assert_no_difference "Post.count" do
      post "/speakers/#{speaker.slug}/posts", params: { theme_id: themes(:one).id }
    end
  ...
  test "音声以外のファイルを送るとPostが作成されない" do
    ...
    assert_no_difference "Post.count" do
      post "/speakers/#{speaker.slug}/posts", params: { audio: text_file, theme_id: themes(:one).id }
    end
