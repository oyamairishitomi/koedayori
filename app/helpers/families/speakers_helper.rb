module Families::SpeakersHelper
  ...
  def last_post_elapsed_label(post)
    elapsed_hours = last_post_elapsed_hours(post)
    return "最後の投稿から1時間未満" if elapsed_hours.zero?

    "最後の投稿から#{elapsed_hours}時間経過"
  end

  def last_post_elapsed_warning?(post)
    last_post_elapsed_hours(post) >= 30
  end
  ...
  def last_post_elapsed_hours(post)
    [ ((Time.current - post.created_at) / 1.hour).floor, 0 ].max
  end
end
